...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall {
            ffi_safe_password_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0)
        }
    }
...
extension PasswordStrength: Equatable, Hashable {}

public func checkStrength(password: String) -> PasswordStrength {
    return try! FfiConverterTypePasswordStrength_lift(
        try! rustCall() {
...
public func generatePassword(
    length: UInt64, includeUppercase: Bool, includeNumbers: Bool, includeSymbols: Bool
) -> String {
    return try! FfiConverterString.lift(
        try! rustCall() {
            uniffi_safe_password_fn_func_generate_password(
                FfiConverterUInt64.lower(length),
                FfiConverterBool.lower(includeUppercase),
                FfiConverterBool.lower(includeNumbers),
                FfiConverterBool.lower(includeSymbols), $0
            )
        })
}
...
private let initializationResult: InitializationResult = {
    ...
    if (uniffi_safe_password_checksum_func_generate_password() != 21913) {
        return InitializationResult.apiChecksumMismatch
    }
